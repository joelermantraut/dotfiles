...
"""
This script launches rofi passmenu utility,
...
respective_fields = ["password", "username", "website"]
...
def launch_rofi(options):
    ...
    string = (
        "list=("
        + options
        + ');password=$(printf "%s\n" "${list[@]}" | rofi -dmenu -i -theme "~/.config/rofi/style_8.rasi" -p "Select: ");echo $password;'
    )
...
def main():
    ...
    if len(fields) > 1:
        if get_window_class() in terminal_list:
            ...
            value = launch_rofi(respective_fields[: len(fields)])
            value = value[:-1]
            # Deletes the last end of line character
